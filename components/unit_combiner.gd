...
# Returns a dict of units grouped by name
func _group_units_in_tier_by_name(tier: int) -> Dictionary:
	...
	# for each unit of the tier, check if name is key in dict and add, otherwise add
	# that units name as a key and append.
...
# returns all combinations found in dict from previous method, 
# combinations are stored as an Array of Arrays, each w/ 3 units
...
func on_buffer_timer_timeout() -> void:
	queued_updates += 1
