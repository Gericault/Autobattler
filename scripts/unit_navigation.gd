...
## Used to draw path lines for debugging
...
func get_next_position(moving_unit: BattleUnit, target_unit: BattleUnit) -> Vector2:
    ...
    # remove unit's current position as solid
    ...
    # of no valid path - stay put and add tile back as solid
    if path.size() == 1 and path[0] == unit_tile:
        astar_grid.set_point_solid(unit_tile, true)
        #return invalid position for handling
        return Vector2(-1, -1)

    # if path is valid, set next position to solid and return new coords.
    ...
    # return as global coord because this means we can decouple
    # AI, anims from game_area.
