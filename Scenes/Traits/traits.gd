...
const TRAIT_UI = preload("res://Scenes/TraitUI/trait_ui.tscn")
...
func _update_traits() -> void:
    ...
    # update/create function clears traits from traits_to_update
    # so that delete orphan can clear remainder
    _move_active_traits_to_top()
    _delete_orphan_traits()

func _update_trait_ui(trait_data: Trait, units: Array[Unit]) -> void:
    # update labels on existing
